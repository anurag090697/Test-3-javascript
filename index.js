...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const totalCostDisplay = document.getElementById("totals");
  let totalCost = 0;
  ...
  items.forEach((item) => {
    ...
    const minus = item.querySelector(".minus");
    const quantity = item.querySelector(".quantity");
    const price = item.querySelector(".quantityValue");
  ...
  function addToCart(item) {
    ...
    namePrice.textContent = `${itemName} - ${amount} X ${price}`;
    buying.appendChild(namePrice);
    cartList.appendChild(buying);
    buying.classList.add("inCart");
  }
  ...
  function removeFromCart(item) {
    const itemName = item.querySelector(".fruitName").textContent;
    const cartItems = cartList.getElementsByTagName("div");
    for (let i = 0; i < cartItems.length; i++) {
      if (cartItems[i].textContent.includes(itemName)) {
        cartList.removeChild(cartItems[i]);
        break;
